    ...
    def __init__(self, args):
        ...
        def create_conv(in_dim, out_dim):
            if self.conv_type == "GCN":
                ...
                return GATConv(in_dim, out_dim, self.n_heads, dropout = args.dropout, concat = False)
...
class SpaSeqNet(torch.nn.Module):
    def __init__(self, args):
        super(SpaSeqNet, self).__init__()
        self.static = StaticNet(DummyArgs(nfeat = args.nfeat, nhid = args.nhid, n_layers = args.n_layers, heads = args.heads, model = args.static_conv, norm = args.norm, dropout = args.dropout))
        hid_dim = args.nhid * 2
    ...
    def forward(self, edge_index_list, x_list):
        ...
        x_list = [self.static(edge_index_list[i], x_list[i]) for i in range(time_len)]
        return x_list, None, None

from torch_geometric.utils import negative_sampling
from DIDA.utils.mutils import bi_negative_sampling
def seq2str(xs):
...
from sild.utils import setup_seed
...
class Trainer:
    ...
    def train_till_end(self, disable_progress = False):
        ...
        with tqdm(range(self.args.max_epoch), disable=disable_progress) as bar:
            for epoch in bar:
                loss = self.train(self.data['train'], epoch = epoch)
                train_auc,val_auc,test_auc = self.test(self.data['train'],type = 'train')
                test_train_auc,test_val_auc,test_test_auc = self.test(self.data['test'], type = 'test')
                metrics = dict(zip('epoch,loss,train_auc,val_auc,test_auc,test_train_auc,test_val_auc,test_test_auc'.split(','),[epoch,loss,train_auc,val_auc,test_auc,test_train_auc,test_val_auc,test_test_auc]))
                ...
                if earlystop.step(**metrics) and epoch >= self.args.min_epoch:
                    break 
